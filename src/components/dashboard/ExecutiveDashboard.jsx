...
export default function ExecutiveDashboard() {
  ...
  const EMPTY = {
    ...
    weekCompare: { thisWeekTotalRevenue:0, lastWeekTotalRevenue:0, percentChange:0, scheduledJobs:0, scheduledRevenue:0 },
    cStoreGallons: [],
    dashboardSquares: {},
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const { data, error } = await getBillboardSummary();
        if (error) {
          ...
          try {
            ...
            if (mounted) setPayload({ ...EMPTY, dashboardKpis: k });
          } catch (e) {
  ...
  const totalGallons = (payload?.dashboardSquares?.totalGallonsAllStores !== undefined)
    ? payload.dashboardSquares.totalGallonsAllStores
    : (payload?.cStoreGallons || []).reduce((s,r) => s + (Number(r.totalGallons)||0), 0);
  ...
  const dashboardKpis = payload?.dashboardKpis ?? { current_tanks:0, customers_lost:0, customers_gained:0, tanks_set:0 };
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      <div style={{ display:'flex', gap: 12, marginBottom: 18 }}>
        <div style={{ flex: 1, minHeight:90, background:'#EDE9FE', borderRadius:8, padding:16 }}>
          <div style={{ fontSize:12, color:'#6D28D9', fontWeight:700 }}>Open Estimates</div>
          <div style={{ fontSize:22, fontWeight:800, marginTop:8 }}>{formatCurrency(0)}</div>
          <div style={{ fontSize:12, color:'#9CA3AF' }}>0 total estimates</div>
        </div>
        <div style={{ flex:1, minHeight:90, background:'#DCFCE7', borderRadius:8, padding:16 }}>
          <div style={{ fontSize:12, color:'#059669', fontWeight:700 }}>Won Estimates</div>
          <div style={{ fontSize:22, fontWeight:800, marginTop:8 }}>{formatCurrency(0)}</div>
          <div style={{ fontSize:12, color:'#9CA3AF' }}>Converted to jobs</div>
        </div>
        <div style={{ flex:1, minHeight:90, background:'#FEE2E2', borderRadius:8, padding:16 }}>
          <div style={{ fontSize:12, color:'#DC2626', fontWeight:700 }}>Lost Estimates</div>
          <div style={{ fontSize:22, fontWeight:800, marginTop:8 }}>{formatCurrency(0)}</div>
          <div style={{ fontSize:12, color:'#9CA3AF' }}>Not converted</div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div style={{ height:240, borderRadius:8, border:'1px solid #E6E6E6', background:'#FFF' }}>
