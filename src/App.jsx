...
/* -----------------------------------------------------------
   Brand + GH Pages-safe asset/base paths
----------------------------------------------------------- */
const BRAND = {
...
const BASE = import.meta.env.BASE_URL || "/";  // <-- CRITICAL for GitHub Pages
const LOGO_URL = `${BASE}site-logo.svg`;       // put in /public
const DATA_JSON_URL = `${BASE}data.json`;      // optional, /public/data.json

/* -----------------------------------------------------------
   Sample data (used until you import/override)
----------------------------------------------------------- */
const sampleHistory = [
...
export default function App() {
  ...
  // Restore from localStorage, then overlay repo /public/data.json
  useEffect(() => {
    try {
  ...
  const exportJson = () => {
    const blob = new Blob([JSON.stringify({ kpi, kpiTargets, history }, null, 2)], { type: "application/json" });
  ...
  const loadRepoData = async () => {
    try {
      ...
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ kpi: base.kpi, kpiTargets: base.kpiTargets, history: base.history, savedAt: new Date().toISOString() }));
      } catch {}
  ...
  // Recompute dependent KPIs on edit
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <KpiCard title="Gross Margin %" value={kpi.grossMargin} target={kpiTargets.grossMargin} formatter={formatPct}>
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={useMemo(() => history.map(d => ({ month: d.month, value: d.grossMargin })), [history])}>
